...
def analyze_symbol(sym, cfg, mkt_bonus, takas_map):
    ...
    m15_raw = download(y, cfg["period_intraday"], "15m")
    if d1_raw is None or m15_raw is None or len(d1_raw) < 180 or len(m15_raw) < 80:
        return None
    ...
    # Weighted early breakout score: 15m trigger + 1H confirmation + 4H setup + daily context
    ...
    return {
        ...
        "stop": st1d["stop"],
        "target": st1d["target"],
        "risk_flags": " | ".join([x for x in [st15.get("flags"), st1h.get("flags"), st4h.get("flags"), st1d.get("flags")] if x]),
        "_frames": {"15M": m15, "1H": h1, "4H": h4, "1D": d1}
    }
...
def main():
    ...
    for r in top[:10]:
        summary += f"• {r['symbol']} skor {r['score']} | para {r['money_flow_15m']} | {r['setup']}\n"
