...
def main():
    ...
    def quality_filter(r):
        """
        V10.3 nihai kalite filtresi:
        Çok aday değil, işlem yapılabilir kaliteye yakın aday üretir.
        """
        score_ok = r.get("score", 0) >= 90

        money15 = r.get("money_flow_15m") or 0
        money1h = r.get("money_flow_1h") or 0
        money4h = r.get("money_flow_4h") or 0
        money_ok = max(money15, money1h, money4h) >= 60

        breakout_ok = bool(r.get("near_breakout")) or bool(r.get("is_breakout"))

        rr = r.get("risk_reward")
        rr_ok = rr is not None and rr >= 0.80
        ...
        if score_ok and money_ok and breakout_ok and rr_ok and (divergence_ok or fib_ok) and not severe_risk:
    ...
    pd.DataFrame(clean).to_csv(OUT / "radar_results_v10_3.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame([{k: v for k, v in r.items() if k != "_frames"} for r in top]).to_csv(
        OUT / "telegram_candidates_v10_3.csv",
        index=False,
        encoding="utf-8-sig"
    )

    if errors:
        pd.DataFrame(errors).to_csv(OUT / "errors_v10_3.csv", index=False, encoding="utf-8-sig")

    summary = (
        f"📡 <b>Gökhan BIST Radar V10.3</b>\n"
        f"Tarih: {started}\n"
        f"Piyasa: <b>{mkt_state}</b>\n"
        f"Taranan: {len(symbols)} | Veri gelen: {len(results)}\n"
        f"Kalite filtresi: Skor≥90 + Para≥60 + Kırılım + R/R≥0.80 + Fib/Uyumsuzluk + ağır risk yok\n"
    ...
    print("CSV:", OUT / "telegram_candidates_v10_3.csv")
