...
def calc_relative_strength(stock_df, xu100_df):
    try:
        stock_20 = stock_df["close"].iloc[-1] / stock_df["close"].iloc[-20] - 1
        xu100_20 = xu100_df["close"].iloc[-1] / xu100_df["close"].iloc[-20] - 1
        return stock_20 - xu100_20
    except Exception:
        return 0


def score_symbol(symbol, xu100_df):
    ...
    if ap["close"] < ap["ema21"] and a["close"] > a["ema21"]:
        score += 20
        reasons.append("EMA21 yeni kırıldı")

    if ap["macd"] < ap["macd_signal"] and a["macd"] > a["macd_signal"]:
        score += 20
    ...
    rs = calc_relative_strength(d1, xu100_df)

    if rs > 0.05:
        score += 20
        reasons.append("XU100'e göre çok güçlü")
    elif rs > 0.02:
        score += 10
        reasons.append("XU100'e göre güçlü")
    elif rs < -0.03:
        score -= 15
        reasons.append("XU100'e göre zayıf")
...
def main():
    try:
        ...
        xu100 = get_data("XU100", "1d", "2y")
        if xu100 is None:
            send_message("XU100 verisi alınamadı. Relative Strength filtresi çalışmayacak.")
            return

        xu100 = add_indicators(xu100)

        results = []

        for i, symbol in enumerate(symbols, 1):
            try:
                item = score_symbol(symbol, xu100)
                if item:
                    results.append(item)
        ...
        for i, a in enumerate(adaylar, 1):
            msg += (
                f"{i}) {a['symbol']} | Skor: {a['score']}\n"
                f"Fiyat: {a['price']} | RSI15: {a['rsi']} | RVOL15: {a['rvol']}\n"
                f"RS XU100: %{a['relative_strength']}\n"
                f"Neden: {', '.join(a['reasons'][:8])}\n\n"
            )

        send_message(msg)

        send_message(f"İlk {GRAFIK_ADET} aday için grafikler hazırlanıyor...")

        for a in adaylar[:GRAFIK_ADET]:
            symbol = a["symbol"]
