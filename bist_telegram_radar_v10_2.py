...
def main():
    ...
    def quality_filter(r):
        """
        V10.2 kalite filtresi:
        Amaç sadece skoru yüksek hisseleri değil;
        para girişi, kırılım, Fibonacci/uyumsuzluk ve risk-ödül dengesini birlikte sağlayan adayları seçmektir.
        """
        score_ok = r.get("score", 0) >= 85

        money_ok = (
            (r.get("money_flow_15m") or 0) >= 60
            or (r.get("money_flow_1h") or 0) >= 60
            or (r.get("money_flow_4h") or 0) >= 60
        )

        breakout_ok = bool(r.get("near_breakout")) or bool(r.get("is_breakout"))

        rr = r.get("risk_reward")
        rr_ok = rr is not None and rr >= 1.00
        ...
        return score_ok and money_ok and breakout_ok and (rr_ok or divergence_ok or fib_ok)
    ...
    pd.DataFrame(clean).to_csv(OUT / "radar_results_v10_2.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame([{k: v for k, v in r.items() if k != "_frames"} for r in top]).to_csv(
        OUT / "telegram_candidates_v10_2.csv",
        index=False,
        encoding="utf-8-sig"
    )

    if errors:
        pd.DataFrame(errors).to_csv(OUT / "errors_v10_2.csv", index=False, encoding="utf-8-sig")

    summary = (
        f"📡 <b>Gökhan BIST Radar V10.2</b>\n"
        f"Tarih: {started}\n"
        f"Piyasa: <b>{mkt_state}</b>\n"
        f"Taranan: {len(symbols)} | Veri gelen: {len(results)}\n"
        f"Kalite filtresi: Skor≥85 + Para≥60 + Kırılım + R/R≥1 veya Uyumsuzluk/Fib\n"
    ...
    for r in top[:10]:
        summary += (
            f"• {r['symbol']} skor {r['score']} | para15m {r['money_flow_15m']} | "
    ...
    print("CSV:", OUT / "telegram_candidates_v10_2.csv")
