...
def analyze_symbol(sym, cfg, mkt_bonus, takas_map):
    ...
    # Dinamik ağırlık korunmuştur. TF score üst sınırı artık 125 olduğu için toplam skor doğal olarak biraz artabilir.
...
def caption(r, tf):
    ...
    return (
        ...
        f"R/R: {r['risk_reward']} | Takas: {r['takas_score']} | {r['takas_note']}\n"
        f"Risk: {risk}"
    )
...
def main():
    ...
    def quality_filter(r):
        """
        V10.4 iki kademeli kalite filtresi:
        A-KALİTE: İşlem yapılabilir kaliteye en yakın aday.
        B-İZLEME: A-Kalite çıkmazsa radarın susmaması için izleme adayı.

        Amaç:
        - Kaliteyi düşürmeden, piyasa yatay/riskli iken tamamen sessiz kalmayı önlemek.
        - Negatif R/R, aşırı RSI, üst fitil, likidite zayıflığı gibi ağır riskleri elemek.
        """
        score = r.get("score", 0)
        ...
        # B-İzleme: piyasa zayıfken tamamen susmamak için kontrollü ikinci kademe
        b_watch = (
            score >= 88
            and best_money >= 55
            and breakout_ok
    ...
    # Hiç aday çıkmazsa, Telegram tamamen boş kalmasın diye sadece en iyi 3 izleme adayını gönder.
    # Bu adaylar işlem sinyali değil, "izleme listesi" olarak işaretlenir.
    if not selected:
        fallback = []
        for r in results:
            rr = r.get("risk_reward")
            ...
            if r.get("score", 0) >= 85 and best_money >= 55 and breakout_ok and rr_value >= 0.25 and fib_ok:
    ...
    pd.DataFrame(clean).to_csv(OUT / "radar_results_v10_4.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame([{k: v for k, v in r.items() if k != "_frames"} for r in top]).to_csv(
        OUT / "telegram_candidates_v10_4.csv",
        index=False,
        encoding="utf-8-sig"
    )

    if errors:
        pd.DataFrame(errors).to_csv(OUT / "errors_v10_4.csv", index=False, encoding="utf-8-sig")

    summary = (
        f"📡 <b>Gökhan BIST Radar V10.4</b>\n"
        f"Tarih: {started}\n"
        f"Piyasa: <b>{mkt_state}</b>\n"
        f"Taranan: {len(symbols)} | Veri gelen: {len(results)}\n"
        f"Kalite filtresi: A-Kalite + B-İzleme; hiç yoksa en iyi 3 C-Sadece İzle\n"
    ...
    for r in top[:10]:
        summary += (
            ...
            f"kırılım {r['breakout_level']} | fib1.618 {r['fib_1618']} | "
    ...
    print("CSV:", OUT / "telegram_candidates_v10_4.csv")
