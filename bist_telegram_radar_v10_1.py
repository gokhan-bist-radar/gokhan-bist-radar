...
def main():
    ...
    pd.DataFrame(clean).to_csv(OUT / "radar_results_v10_1.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame([{k: v for k, v in r.items() if k != "_frames"} for r in top]).to_csv(
        OUT / "telegram_candidates_v10_1.csv",
        index=False,
        encoding="utf-8-sig"
    )

    if errors:
        pd.DataFrame(errors).to_csv(OUT / "errors_v10_1.csv", index=False, encoding="utf-8-sig")

    summary = (
        f"📡 <b>Gökhan BIST Radar V10.1</b>\n"
        f"Tarih: {started}\n"
    ...
    print("CSV:", OUT / "telegram_candidates_v10_1.csv")
